...
func InitTracker(
	...
	for j, s := range jobMap {
		...
		if !s.isDone() {
			...
			metrics.TasksInFlight.WithLabelValues(j.Experiment, j.Datatype, string(s.State())).Inc()
		}
	}
	t := Tracker{
...
func (tr *Tracker) AddJob(job Job) error {
	...
	if ok {
		if s.isDone() {
			...
			metrics.TasksInFlight.WithLabelValues(job.Experiment, job.Datatype, string(s.State())).Dec()
...
func (tr *Tracker) GetState() (JobMap, Job, time.Time) {
	...
	for j, s := range tr.jobs {
		...
		if (tr.expirationTime > 0 && time.Since(updateTime) > tr.expirationTime) ||
			...
			if !s.isDone() {
				// If job didn't complete, the InFlight metric needs to be updated.
				metrics.TasksInFlight.WithLabelValues(j.Experiment, j.Datatype, string(s.State())).Dec()
